...
from bokeh.models import  ColumnDataSource, Button, PrintfTickFormatter, SingleIntervalTicker, Range1d, Text, Slider, RangeTool
...
#COLORS = ['#a6cee3', '#b2df8a','#1f78b4', '#33a02c', '#fb9a99', '#e31a1c', '#fdbf6f', '#ff7f00', 'gray']
COLORS = ['gray', '#abcee3', '#fb9a99', '#fdbf6f', '#b2df8a', '33a02c', '#ff7f00' ,'1f78b4' ,'e31a1c' ]
# グラフの設定
WIDTH = 380
HEIGHT = 350
WIDTH_LEGEND = 50
...
p8 = figure(title="Contibution to x-axis Graph", height=220, width=600, tools="pan,box_zoom,reset,save", x_range=(10,15))
p9 = figure(title="select tool", height=220, width=600, tools="", toolbar_location=None )
...
#continuous : マウスを動かすたびに選択を行うかどうかを制御する
...
def slider_callback(attr, old, new, row, col):
    ...
    #p8の更新
    for renderer in p8.renderers:
        p8.renderers.remove(renderer)
    for renderer in p9.renderers:
        p9.renderers.remove(renderer)
    ...
    p8.line(x=xs, y=ys)
    p8.circle(x=xs, y=ys, size=3, color='red')
...
original_layout = column(row(column(p1,b1),p2,p11),
                row(column(p6,b2), p7, column(p8, p9)))
