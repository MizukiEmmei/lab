...
# position_name = [name.replace('Fairbanks North Star', 'North Star') for name in position_name]
...
COLORS = ['gray', '#1f78b4', '#b2df8a', '#33a02c', '#fb9a99', '#e31a1c', '#fdbf6f', '#ff7f00', '#cab2d6', '#6a3d9a', 'black']
# グラフの設定
WIDTH = 380
HEIGHT = 350
WIDTH_LEGEND = 50
...
p8 = figure(title="Contibution to x-axis Graph of selected renge", height=300, width=1000, tools="pan,box_zoom,reset,save", x_range=(10,15))
p9 = figure(title="Overall Contibution to x-axis Graph", height=300, width=1000, tools="pan,box_zoom,reset,save")
...
def p6_callback(attr, old, new):
    ...
    if len(inds) > 1:
        ...
        output_df = pd.DataFrame({'px': s_p6p7.data['px'],
                'py': s_p6p7.data['py'],
                'name' : s_p6p7.data['name'],
                'color' : s_p6p7.data['color']
                })

        output_df.to_csv('America_plot.csv', index=False)
