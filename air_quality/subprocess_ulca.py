...
df_origin = pd.read_csv(original_data_path, header=0, index_col = [0], parse_dates=True, usecols=[1,2,6,7,8,9,10]) #/use_cols : 1~6行目を読み込む
...
# ラジオボタンの設定
#rb2は使わない,おそらくデータ収集の際にたまに使っていたのだろう
rb1 = RadioGroup(labels=LABELS_RADIO, active=0)
...
def call_ulca_p6():
    ...
    #selected_columns = ['index', 'cluster_id', 'CO2', 'NO', 'Ozone', 'PM10', 'PM2.5']
    # selected_df = df_p6_10.loc[:, selected_columns]
    # df_str = selected_df.to_csv(index=False)
    # subprocess.run(['python3.10', '-i', '-c', f'from test_sub_space import process_dataframe; process_dataframe("""{df_str}""")'])
    # print(selected_df)
    # subprocess.run(['python3.10', '-i', '-c', f'from test_sub_space import process_dataframe; process_dataframe("""{df_str}""")'])
...
def make_tensor_data(time_list, space_list, flag):
    T2 =len(time_list)
    S2 = len(space_list)
    if flag == 0:
        filtered_df_time = df_origin_time[df_origin_time.index.isin(time_list)] #time_indexから抽出
        filtered_df_time = filtered_df_time[filtered_df_time['space'].isin(space_list)] #space_indexから抽出
        X = mf.unfold(filtered_df_time[variables],0,T2,S2,V) #T*(S*V)
        print(X)
    else:
        filtered_df_space = df_origin_space[df_origin_space.index.isin(space_list)] #space_indexから抽出
        filtered_df_space = filtered_df_space[filtered_df_space['time'].isin(time_list)] #time_indexから抽出
        X = mf.unfold(filtered_df_space[variables],1,T2,S2,V) #S*(T*V)
        print(X)
