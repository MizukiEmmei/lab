...
from bokeh.models import  ColumnDataSource, Button, PrintfTickFormatter, SingleIntervalTicker, Range1d, Text, Slider
...
df_xy = pd.read_csv(original_data_path, usecols = [3,4,5])
#緯度経度,xyが逆になっていることに注意
position_y = df_xy["Instance_x"][:55].to_numpy()
position_x = df_xy["Instance_y"][:55].to_numpy()
position_name = df_xy["Instance_name"][:55].to_numpy()
position_name = [name.replace('District of Columbia', 'of Columbia') for name in position_name]
position_name = [name.replace("Prince George's", "George's") for name in position_name]
position_name = [name.replace('Los Angeles', 'LA') for name in position_name]
position_name = [name.replace('San Bernardino', 'San Bern') for name in position_name]
position_name = [name.replace('San Francisco', 'San Fra') for name in position_name]
position_name = [name.replace('Santa Barbara', 'Santa Bar') for name in position_name]
...
df_origin = pd.read_csv(original_data_path, header=0, index_col = [0], parse_dates=True, usecols=[1,2,6,7,8,9,10]) #/use_cols : 1~6行目を読み込むXX 
T = (int)(len(df_origin)/S)
variables = ['CO2', 'NO', 'Ozone', 'PM10', 'PM2.5']

#make_tensor_dataで使用
# df_origin_time = pd.read_csv(original_data_path, header=0, index_col = [0], parse_dates=True, usecols=[1,2,6,7,8,9,10])
# df_origin_space = pd.read_csv(original_data_path, header=0, index_col = [1], parse_dates=True, usecols=[1,2,6,7,8,9,10])
# df_origin_space['time'] = pd.to_datetime(df_origin_space['time'], format='%Y%m%d').dt.strftime('%Y-%m-%d')
# df_origin_space = df_origin_space.sort_values(['space', 'time'])

#マルチインデックス（時間→空間）で読み込み
df_ts = pd.read_csv(original_data_path, index_col=[0, 1], parse_dates=True, header=0, usecols=[1,2,6,7,8,9,10])
...
p1_selected_id = 0
p6_selected_id = 0
selected_time_index = df_ts_means.index #(['2014-04-01', '2014-04-02', '2014-04-03', '2014-04-04',・・・・
selected_space_index = df_st_means.index #[1,2,4,5,6,7,・・・・]
...
COLORS = ['gray', '#a6cee3', '#b2df8a','#1f78b4', '#33a02c', '#fb9a99', '#e31a1c', '#fdbf6f', '#ff7f00', '#cab2d6', '#6a3d9a', 'black']
...
p2 = figure(title="Time domain plot", x_axis_label='timestamp', y_axis_label='cluster', width=WIDTH, height=410, sizing_mode='fixed',
...
p7_x_range_fixed = Range1d(start=-150, end=-70)  # 適切な範囲を指定
p7_y_range_fixed = Range1d(start=24, end=66)  # 適切な範囲を指定
p7 = figure(title="Space domain plot", x_axis_label='longitude', y_axis_label='latitude', width=p2.width, height=p2.height, sizing_mode='fixed',
            outline_line_color='black', background_fill_color="#fafafa", min_border_bottom=MIN_BORDER_BOTTOM_p7, min_border_left=MIN_BORDER_LEFT_p7, tools=TOOLS_2, x_range=p7_x_range_fixed, y_range=p7_y_range_fixed)
...
#ULCAの結果をプロット
p11_x_range_fixed = Range1d(start=-15, end=55)  # 適切な範囲を指定
p11_y_range_fixed = Range1d(start=-15, end=25)  # 適切な範囲を指定
...
#             outline_line_color='black', background_fill_color="#fafafa", x_range=s_p12.data['feat_names'])
# p12.xaxis.major_label_orientation = "vertical"

# p13 = figure(title="contribution to Y axis", width=400, height=200, sizing_mode='fixed',
#             outline_line_color='black', background_fill_color="#fafafa", x_range=s_p13.data['feat_names'])
...
s_p6p7.data['name'] = position_name
...
p7.scatter('px', 'py', color='color', size='size', alpha=alpha, legend_group='label', marker='markers', 
            source=s_p6p7, nonselection_alpha=nonselection_alpha)

# p8.rect('px', 'py', 1, 1, fill_color='color', line_color = 'black', alpha=alpha,
...
#ULCAの寄与度TOP10をプロット
# p12.vbar(x='feat_names', top='contribution', color='gray', width=VBAR_WIDTH, source=s_p12)
# p13.vbar(x='feat_names', top='contribution', color='gray', width=VBAR_WIDTH, source=s_p13)
p12.hbar(y='feat_names', left=0, right='contribution', color='gray', height=VBAR_WIDTH, source=s_p12)
p13.hbar(y='feat_names', left=0, right='contribution', color='gray', height=VBAR_WIDTH, source=s_p13)
p12.xaxis.visible = False  # y軸のメモリを非表示にする
p13.xaxis.visible = False  # y軸のメモリを非表示にする
...
p11.add_tools(HoverTool(tooltips=[('name', '@name')]))
...
def update_s_p1p2():
    ...
    p1.scatter('x', 'y', color='color', size='size', fill_alpha='alpha', marker='markers', legend_group = 'label', 
                    source=s_p1p2, nonselection_alpha=nonselection_alpha)
    s_p3.data = {'py': [], 'value': [], 'color': []}
    s_p4.data = {'px': [], 'value': []}
    s_p5.data = {'py': [], 'value': [], 'color': []}
    #print(s_p1p2.data)
...
def update_s_p6p7():
    ...
    df2['cluster_id'] = 0
    df2['color'] = 'gray'
    df2['name'] = [position_name[i-1] for i in df2['space']]
    df2 = df2.reset_index()
...
def call_ulca_p1():
    ...
    space_name = s_p6p7.data['name']
    feat_names = [f"{i} : {v}" for i in space_name for v in variables]
    ...
    execute_ulca(X, cluster_id_list, feat_names, cluster_num, time_list)
...
def call_ulca_p6():
    ...
    execute_ulca(X, cluster_id_list, feat_names, cluster_num, space_name)
    
    
# スライダーのコールバック関数
#スライダーの値に応じて重みを更新してULCAを再実行
def slider_callback(attr, old, new, row, col):
    global p11_x_range_fixed, p11_y_range_fixed, g_name_list
    A[row][col] = new
    w_tg = dict() 
    w_bg = dict()
    w_bw = dict()
    ...
    df['name'] = g_name_list
    s_p11.data = df
    # text.text = 'ULCA result plot (alpha=' + str(1 / ulca.get_final_cost()) + ")"
    ...
    #p12の更新(Contribution to x)
    df_12 = pd.DataFrame({'contribution' : ulca.M[:,0], 'feat_names' :g_feat_names})
    # contribution列の絶対値でソートし、上位10件を抽出
    # df_12 = df_12.iloc[df_12['contribution'].abs().argsort()[-10:]]
    df_12.to_csv('x_df.csv', index=False)
    df_12 = df_12.iloc[df_12['contribution'].abs().argsort()[-10:]]
    # FactorRange を使用して x_range を設定
    s_p12.data = df_12
    #p12.x_range = FactorRange(*df_12['feat_names'])
    p12.y_range.factors = s_p12.data['feat_names'].tolist()
    #p13の更新
    df_13 = pd.DataFrame({'contribution' : ulca.M[:,1], 'feat_names' :g_feat_names})
    # contribution列の絶対値でソートし、上位10件を抽出
    df_13 = df_13.iloc[df_13['contribution'].abs().argsort()[-10:]]
    # FactorRange を使用して x_range を設定
    s_p13.data = df_13
    #p12.x_range = FactorRange(*df_12['feat_names'])
    p13.y_range.factors = s_p13.data['feat_names'].tolist()

   
def execute_ulca(X, y, feat_names, cluster_num, name_list):
    global g_X ,g_y, g_feat_names, g_cluster_num, g_name_list, v, A, p11_x_range_fixed, p11_y_range_fixed
    g_X = X
    g_y = y
    ...
    p11.title.text = text.text
    #p12の更新(Contribution to x)
    df_12 = pd.DataFrame({'contribution' : ulca.M[:,0], 'feat_names' :feat_names})
    # contribution列の絶対値でソートし、上位10件を抽出
    df_12 = df_12.iloc[df_12['contribution'].abs().argsort()[-10:]]
    # FactorRange を使用して x_range を設定
    s_p12.data = df_12
    ...
    p12.y_range.factors = s_p12.data['feat_names'].tolist()
    #p13の更新
    df_13 = pd.DataFrame({'contribution' : ulca.M[:,1], 'feat_names' :feat_names})
    # contribution列の絶対値でソートし、上位10件を抽出
    df_13 = df_13.iloc[df_13['contribution'].abs().argsort()[-10:]]
    # FactorRange を使用して x_range を設定
    s_p13.data = df_13
